...
void FeatureData::ReadFeatureDataA(
    ifstream& is, int npt, int locDim, int desDim, int szFeatureName) {
  int i, j;
...
void FeatureData::CopyToFeatureData(FeatureData& fd) {
  if (getFeatureNum() > 0) {
    ...
    // getLocationData().bsize()); memcpy(fd.getDescriptorData().data(),
    // getDescriptorData().data(), getDescriptorData().bsize());
  }
}
...
void FeatureData::SetFeatureClip(FeatureData& src,
                                 int nsf,
                                 int index[],
                                 int subset) {
  ReleaseFeatureData();
...
int FeatureData::ReadSIFTB_DES(const char* szFile,
                               unsigned char* buf,
                               int nmax) {
  sift_fileheader_v2 sfh;
  int fd = open(szFile, O_RDONLY, S_IREAD);
...
int FeatureData::ReadSIFTA_DES(const char* szFile,
                               unsigned char* buf,
                               int nmax) {
  FeatureData fd;
  fd.ReadSIFTA(szFile);
  ...
  nmax = std::min(nmax, fd.getFeatureNum());
  if (nmax > 0) {
...
int FeatureData::appendSIFTB(const char* szFile, int pos) {
  ...
  if (npoint == SIFT_NAME && IsValidVersionName(nLocDim)) {
    ...
    read(fd,
         (float*)_locData->data() + pos * nLocDim,
         nLocDim * npoint * sizeof(float));
    read(fd,
         (unsigned char*)_desData->data() + pos * nDesDim,
...
void FeatureData::SaveLocationFile(const char* szFeatureFile) {
  ...
  float* p = &loc[0];
  int fid =
      open(szFeatureFile, O_CREAT | O_WRONLY | O_TRUNC, S_IREAD | S_IWRITE);
  if (fid == -1) return;
...
void FeatureData::ShrinkLocationData(int ndim, int npoint) {
  if (_locData)
    _locData->shrink(
        ndim,
        npoint < 0 ? _locData->npoint() : std::min(npoint, _locData->npoint()));
}
