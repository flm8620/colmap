...
void ImageView::LoadImage(int idx) {
  ...
  const std::vector<AbstractFeature>& features =
      image_scene->ImageToFeatures()[idx];
...
void ImageView::fitViewAllObject() {
  ...
  const QRectF rect_large(rect.topLeft() - size * 10,
                          rect.bottomRight() + size * 10);
...
void ImageView::UpdateFeaturesVisibility() {
  if (!show_features) {
    for (const auto& it : features) it.second->setVisible(false);
  } else {
...
void ImageView::DrawEpipolarLine(double a,
                                 double b,
                                 double c) {  // ax + by + c = 0
