...
class Line : public QWidget {
    ...
    for (const auto& m : matches) {
      ...
      if (ok1 && ok2) {
        ...
        if (p1.x() < 0.f || p1.y() < 0.f ||
            p1.x() > left_view->rect().width() ||
            p1.y() > left_view->rect().height())
          continue;
        if (p2.x() < 0.f || p2.y() < 0.f ||
            p2.x() > right_view->rect().width() ||
            p2.y() > right_view->rect().height())
          continue;
 ...
 public:
  ...
  void SetShowMatchLines(bool checked) { show = checked; }
};
...
class ImageHolder : public QWidget {
 ...
 public:
  ...
  void resizeEvent(QResizeEvent*) override { line->setGeometry(rect()); }

  void LoadImageLeft(int idx) {
    left_view->LoadImage(idx);
    if (right_view->current_img_idx() >= 0) {
      fundamental_matrix = ImageInfo::calcFundamentalMatrix(
          scene->Images()[idx], scene->Images()[right_view->current_img_idx()]);
    }
  }
  ...
  void LoadImageRight(int idx) {
    ...
    if (left_view->current_img_idx() >= 0) {
      fundamental_matrix = ImageInfo::calcFundamentalMatrix(
          scene->Images()[left_view->current_img_idx()], scene->Images()[idx]);
    }
  }
  ...
  void SetMatchesToLines() { line->set_matching_lines(matches); }
  ...
  void SetShowMatchLines(bool checked) { line->SetShowMatchLines(checked); }

  void SetShowEpipolar(bool checked) { show_epipolar = checked; }
};
