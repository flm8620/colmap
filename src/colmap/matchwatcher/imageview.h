...
class ImageView : public QGraphicsView {
 ...
 public:
  ...
  void set_scene(ImageScene* image_scene) { this->image_scene = image_scene; }
  ...
  bool GetFeaturePos(int id, QPointF& pos) {
    if (id < 0 || id >= features.size()) return false;
  ...
  void SetMaxSize(double size) { max_feature_size = size; }

  void SetShowFeatures(bool show) { show_features = show; }
