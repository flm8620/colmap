...
//image2------------------------------ Ues member funcation
// -----MatchFile::GetMatchCount-------------------------------- int NM;
// //number of putative matches int NF;		//number of inlier matches
...
//matches---------------------------------------------------- use member
// function ----MatchFile::GetPMatch---------------------------------------- int
// FC;		//feature count of image2, input int NM;		//number
// of putative matches, output Points<int>	matches;// feature index of
// putative matches, output MatchFile mat(image1_full_path); //image1's match
// database if(mat.IsValid() && mat.GetPMatch(image2_full_path, FC, NM,
// matches)) {/* we have it*/};

//////////////////////////////////////////////////////////////////////////
//+++To write puattive
//matching---------------------------------------------------------- Use static
// function ----MatchFile::WritePMatch---------------------------------------
...
//matrix 		matches[0][] are the indices of image1 		matches[1][] are the indices of
//image2
...
class MatchFile {
 ...
 public:
  ...
  static void GetRelativePath(const char* path1,
                              const char* path2,
                              char* relative_path);
 ...
 public:
  ...
  int GetIMatch(const char* image_path,
                int FC,
                TwoViewGeometry& tvg,
                Points<int>& inliers);
  int GetPMatchR(const char* image_path, int FC, int& NM, Points<int>& matches);
  int GetIMatchR(const char* image_path,
                 int FC,
                 TwoViewGeometry& tvg,
                 Points<int>& inliers);
 ...
 public:
  //////////////////////////////write matches
  void WriteIMatch(const char* image_match,
                   int reverse,
                   int NM,
                   TwoViewGeometry& tvg,
                   Points<int>& inliers);
  void WritePMatch(const char* image_match,
                   int FC,
                   int NM,
                   Points<int>& matches,
                   int reverse);
 ...
 public:
  ////////////////////static functions
  static void WriteIMatch(const char* image1,
                          const char* image2,
                          int NM,
                          TwoViewGeometry& tvg,
                          Points<int>& inliers);
  static void WriteIMatch(MatchFile* mat,
                          const char* image1,
                          const char* image2,
                          int NM,
                          TwoViewGeometry& tvg,
                          Points<int>& inliers);
  static void WritePMatch(const char* image1,
                          const char* image2,
                          int fc1,
                          int fc2,
                          int NM,
                          Points<int>& matches);
  static void WriteFMatch(const char* image1,
                          const char* image2,
                          int NF,
                          int* index1,
                          int* index,
